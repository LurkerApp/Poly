...
# Put/Call ratio per expiration for chart overlay
...
# ── Overall put/call ratio (nearest expiry) ───────────────────
...
# ── IV approximation from straddle price ─────────────────────
...
def print_results(symbol, current_price, moves, hv, pc_ratio, iv_hv_spread):
    ...
    print(f"{'='*60}")
    print(f"  {'Expiration':<14} {'DTE':<6} {'±Move':<10} {'%':<8} {'IV%':<8} {'P/C':<6} {'Range'}")
    print(f"  {'-'*65}")
    for m in moves:
        print(
            f"  {m['expiration']:<14} "
